...
> **Threat Score:** a per-user risk pill in profiles, worked out from your own server's warning history. It is **advisory only and never takes an action by itself**, and it needs the **full Premium** plan: it is not part of the cheaper **Premium BYOK** plan. Off until you switch it on in `/settings`. Do not confuse it with ThreatNet below, which is the separate cross-server signal.

> **ThreatNet (Cross-Server Threat Network):** a **core, on-by-default** safety feature. The **advisory band is free for every server** (your staff decide what to do; it never auto-acts). **Full Premium** adds the **richer cross-server view** and, optionally, **ThreatNet auto-protect**: automatically banning a joining user who meets a cross-server risk threshold you choose (off by default; turn it on with `/threatnet autoban on` or in `/settings → Security → ThreatNet`). Neither is part of the cheaper **Premium BYOK** plan. Severity-only sharing (never the offence type, reasons, content, or which server acted); individuals can opt out of profiling in the [web portal](https://serverassistant.wandweb.co), and auto-protect does not act on anyone who has opted out, apart from the most serious corroborated cases covered by a safety exception.
