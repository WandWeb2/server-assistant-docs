...
<p style="color:var(--fg-1);font-size:.9rem;line-height:1.55;"><strong>Pass and you're in.</strong> A correct answer grants the verified role you chose in settings; a wrong answer is a friendly retry, Reception <strong>never bans</strong> someone for fumbling the question. Because the <em>only</em> thing to do at the desk is verify, it doubles as a <strong>self-healing honeypot</strong>: the channel stays locked as a spam trap, and if its settings ever drift, the bot restores them on its own so the trap keeps working. Anyone who posts in Reception has their message removed, with a note explaining why, since the channel is for verification only (brand-new, untrusted accounts that post instead of verifying are removed outright, and the trap tightens during a raid). <strong>The gate holds either way:</strong> Reception checks that nothing else is auto-granting your verified role (Discord Onboarding, another bot, and the like), warns you if it finds one, and strips the role back from anyone who gets it without passing the desk. <strong>Free</strong> servers now get the full desk, including a <strong>banner</strong> created once automatically from the bot's read of your community, and a greeting you word yourself. <strong>Premium</strong> adds a custom <strong>banner image</strong> of your own, a custom <strong>receptionist name and persona</strong>, and a <strong>concierge</strong>: instead of simply tidying a member's post away, a member who reaches out in Reception is moved into their own private ticket where SAi answers from the bot's product knowledge one-to-one, keeping the desk clear for new arrivals.</p>
