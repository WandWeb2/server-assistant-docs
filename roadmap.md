...
<div class="build-queue">
  ...
  <div class="band" id="band-blue">
    ...
    <details class="card"><summary>😄 Emote commands</summary><span class="desc">Lightweight expression commands any member can use — <code>/hug</code>, <code>/wave</code>, <code>/pat</code>, <code>/highfive</code> and friends — that post a friendly action (with an optional <code>@member</code> target) to liven up chat. Owners pick which are enabled and where, with the same per-channel and rate-limit controls as the rest of the bot, so they add personality without becoming spam.</span></details>
...
<!-- Feature packs — collapsible themed bundles (details.pack, collapsed by
  ...
  <details class="pack">
  <summary class="pack-h">🎉 Engagement &amp; Community <span class="pack-count">11 cards · 4 new · 1 upgrade</span></summary>
  ...
  <details class="card"><summary>😄 Emote / reaction commands <span class="new-pill">🆕 New</span></summary><span class="desc">Expression commands any member can use — <code>/cry</code>, <code>/hug</code>, <code>/wave</code>, <code>/laugh</code> and friends — that post a fitting GIF to liven up chat. Run bare (<code>/cry</code>) and <strong>AI picks the theme</strong> from your server type, the channel, and the last few messages — then pulls a <strong>random</strong> matching GIF so it's never the same one twice. Or define the genre yourself — <code>/cry naruto</code>, <code>/cry goku</code>, <code>/cry wow</code> — anything goes, since the genre is a live GIF search. Target someone (<code>/hug @member</code>) for two-person actions, and set a <strong>personal go-to subject</strong> so your emotes lean your way by default (e.g. always Naruto). <strong>AI theme-picking is a Premium perk (with a trial); when AI tokens lapse it falls back to a plain random GIF</strong>, so the commands always work. <strong>Moderation-safe</strong>: respects your NSFW/age-gate, allowed channels and rate limits, all tunable in <code>/settings</code> — fun that still follows your server's rules.</span></details>
