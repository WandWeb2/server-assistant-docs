...
<!-- Feature packs — collapsible themed bundles (details.pack, collapsed by
  ...
  <details class="band-shipped">
    ...
    <details class="card">
      <summary>😄 Emote commands <span class="shipped-pill">✅ Shipped v6.6.0</span></summary>
      <span class="desc">Members post a fitting GIF with <code>/emote</code>, <code>/e</code>, <code>/cry</code>, <code>/hug</code>, <code>/dance</code> and more — autocomplete over hundreds of emotes (cry, dance, laugh… plus aliases like <code>lol</code>, <code>ty</code>). Run a bare emote and the bot <strong>AI-picks a theme</strong> fitting your server + the moment, then pulls a <strong>random</strong> matching GIF; or set it yourself (<code>/cry anime</code>, <code>/cry wow</code>) or pick a personal default with <code>/emotestyle</code>. <code>/hug @member</code> aims it at someone. The auto-theme runs on your server's <strong>AI allowance including the free trial</strong> (not Premium-only) and falls back to a plain random GIF when that's used up. <strong>Moderation-safe</strong> — safe-search on, plus an enable toggle, channel allowlist, rate limit, NSFW option and an off-by-default flavor pack in <code>/settings → Emotes</code>. GIFs by KLIPY. <a href="{{ site.url }}{{ site.baseurl }}/wiki/emotes/">Full guide →</a></span>
    </details>
