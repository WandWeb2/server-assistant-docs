...
- **Advisory by default; optional Premium automated action.** For every server, the network score is **advisory** — it surfaces a risk picture to human staff, who decide what (if anything) to do. **Premium servers may additionally opt in to ThreatNet auto-protect**, which **automatically bans a user when they join** if that account carries a **serious, corroborated** cross-server safety record (the highest band — serious *and* corroborated across **multiple independent** servers). Auto-protect is **off by default** and is switched on per server by a server administrator. Where it is enabled, the resulting ban is an **automated decision**: the affected individual is told the decision was automated and how to obtain **human review** and appeal (see *Individual opt-out and appeals* and the [Privacy Policy]({{ site.url }}{{ site.baseurl }}/privacy/)); individuals who have opted out of network profiling are **never** auto-actioned; and the decision to enable, configure and operate auto-protect — and final accountability for it — rests with the server (see *Server-owner responsibility*). <!-- LEGAL REVIEW (NEW — AUTOMATED ACTION, needs sign-off): This bullet REVERSES the prior "advisory only — never auto-actions" stance for Premium opt-in servers. A fully automated ban producing a significant effect on an individual engages GDPR Art. 22 — mitigations shipped: opt-in only + OFF by default, hard "high"+corroborated threshold (serious AND ≥2 independent servers), opt-out always honoured, automated-decision notice to the user with a human-review/appeal route, audit log, no re-emission of a network signal from an auto-ban. Residual: a wrongly-banned END USER never accepted these Terms, so this clause doesn't bind them (defamation / Art. 22 / ACL exposure) — the high+corroborated floor + appeal route + advisory-grade accuracy are the real mitigations. Confirm with the owner AND a lawyer before publishing. -->
