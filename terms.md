...
- **Participation is core functionality, not optional.** By inviting the Bot, your server **contributes** minimized abuse signals to the network and is **protected by** it. **There is no server-level opt-out** — the network only works when every protected server participates, and contribution and protection are inseparable. Only minimized aggregates (counts, recency, category bands, match indicators) are shared — never your staff's free-text reasons, never your members' message content, and never which server acted.

- **You must tell your members.** Because participation cannot be switched off, the server owner is responsible for making members aware of the Bot's [Privacy Policy]({{ site.url }}{{ site.baseurl }}/privacy/) and that the server takes part in the Cross-Server Threat Network (see *Server-owner responsibility*). Where your members are in a region whose law requires it, you are responsible for giving them any notice or obtaining any consent that applies to them. <!-- LEGAL REVIEW (PARTIALLY RESOLVED): A server owner accepting these Terms binds the server to no-opt-out contribution — consistent with the controller-role disclosure and the legal framing in the Privacy Policy (Australian Privacy Act / APPs primary; GDPR legitimate interest secondary). RESIDUAL, needs a lawyer: this Terms acceptance binds the SERVER, but it does NOT by itself create individual-member consent. Where the shared signals are "sensitive information" (AU, APP 3.3) / criminal-offence data (GDPR Art. 10), member-level consent or a specific exception may be required and a server owner cannot waive it for their members. The member-notice obligation above is the cheapest mitigation but does not fully cure the consent gap. See .omp/threat-network-PIA-LIA.md §A-RISK and .omp/RISK-REGISTER.md. -->
