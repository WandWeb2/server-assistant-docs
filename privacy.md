...
| **AutoMod AI second-opinion** *(Premium, opt-in)* | The text of a single AutoMod-flagged message + which filter matched. Only **word-filter and spam** hits are ever sent; link and mention hits are not, and a message that matched no filter is never sent at all. Be clear about the limit: on a server that has switched this on, **every** word-filter or spam match is sent, including an obvious one. The filter family is the narrowing, not how clear-cut the match looked. **Discord messages only today** — see *Minecraft chat and other in-game text* below | Off (opt-in via `/automod → AI Review`) |
