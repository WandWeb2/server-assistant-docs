...
One further point, because the description above is easy to read too narrowly. Alt-guard **reads** no network data, but it does **feed** it. As far as the rest of the Bot is concerned a ban is a ban, so an alt-guard automatic ban contributes a signal to the Cross-Server Threat Network in the same way, and at the same severity, as a ban your staff decide on themselves. A ban that no human ordered still becomes part of that person's cross-server record, and can therefore count toward the record other servers act on. We think you should know that, because "local to one server" describes where alt-guard looks, not where its result ends up.
