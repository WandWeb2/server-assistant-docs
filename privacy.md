...
This is the one place a short extract of flagged **in-game** text is written to our database; the moderation-actions log above deliberately holds none. A decision record is deleted **90 days after the decision is closed**, whether a staff member closed it or it closed itself.

Some decisions carry a **deadline**. If that deadline passes with nobody having acted, the Bot does not simply drop the item: it **warns first**, recording the warning in the web portal's activity feed and sending it on as a portal notification and as a direct message to a staff member, and only if a further **24 hours** goes by with still no action does it close the item itself, marked **expired** rather than decided. The 90-day deletion clock then runs from that automatic closure. A decision that carries **no deadline** is different: it stays open at any age and is only ever closed by a person. Either way, nothing disappears from the queue unannounced, which matters to us more than tidiness does.
...
| **AutoMod AI second-opinion** *(Premium, opt-in)* | The text of a single AutoMod-flagged message + which filter matched. Only **word-filter and spam** hits are ever sent; link and mention hits are not, and a message that matched no filter is never sent at all. **The narrowing is the filter, not the certainty**: on a server that has switched this on, every word-filter or spam match is sent, an obvious one included, because judging how clear-cut it is is what the check is for. **Discord messages only today** — see *Minecraft chat and other in-game text* below | Off (opt-in via `/automod → AI Review`) |
...
- **A record of every XP change (the activity log):** each time a member's XP moves, we append one entry holding **when** it happened, the member's **Discord user ID and username**, the **amount** and the **resulting total**, and a short line saying **what caused it**, that is, chat activity, a voting or review reward, an adjustment made by our operator, or a **Crestbound** action such as opening a booster pack. It is not shown in your server's web portal or anywhere in Discord; it exists so our operator has a readable history of what changed when someone asks about their balance or disputes it. This log is held separately from the XP totals themselves and is **deleted automatically after 180 days**.
...
**Retention.** XP, level, message counts, reputation and the account-wide XP wallet are retained for as long as the member's account-wide wallet is in use, that is, until the data is deleted on request. There is no automatic expiry for those totals. The **activity log of individual XP changes** described above is the exception, and does expire: each entry is deleted automatically **180 days** after it is written. You may contact us via [`/support`]({{ site.url }}{{ site.baseurl }}/support/) to request access to, or deletion of, the XP data we hold about you.
...
- **A single message**, only once it has already matched a filter, and only a match on the **word filter** or the **spam** rule: link and mention hits are never sent, and neither is anything that matched no filter. This is a limit on which filters can cause a send, not on how clear-cut the match was. Within those two rules every match is sent, an obvious one included, so we do not claim a certainty screen that does not exist.
...
| Staff decision records (the extract of the Discord message, appeal, or in-game text a decision is about — in-game text meaning chat, a sign, a book, an item name, a mob name tag, a `/me` emote, a username, a private message, or a private ask to the in-world companion) | Deleted **90 days after the decision is closed**, by a staff member or by the automatic close described above. A decision with a **deadline** that passes unactioned is warned, then closed automatically as **expired** 24 hours later, and the 90-day clock runs from there. A decision with **no deadline** stays open at any age, and is only closed by a person |
...
| XP activity log (one entry per XP change: user ID, username, amount, resulting total, and what caused it) | Auto-deleted after **180 days**, and on request before that |
