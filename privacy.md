...
| **🛡️ Cross-Server Threat Network** | Minimized abuse signals (a pseudonymous Discord user ID, ban/kick counts + recency, a **severity level** — e.g. minor / serious, AltGuard fingerprint-match indicator, account-age/join-velocity) shared across protected servers as **aggregates and a severity level only** — never the offence type/category, never reasons, never message text, never which server acted. See **Cross-Server Threat Network** below | On (core feature, no opt-out) |
...
The safeguards in this section — strict severity-only minimization, the rights-request route below, advisory-only use, anti-abuse corroboration, and a hard 12-month retention cap — are what keep this handling proportionate. Server owners are also asked to inform their members that the server participates (see our [Terms of Service]({{ site.url }}{{ site.baseurl }}/terms/)).
