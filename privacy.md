...
**Minecraft servers running our bridge.** Two bridge features send in-game content to the AI providers above, on whichever provider serves that server's tier. **Chat translation** (off by default; a server operator turns it on by setting a target language in `/mcdc`) passes bridged chat through the translation model in both directions — in-game lines on their way to Discord, and Discord messages on their way in-game. **In-game `@sai`** (Premium, off by default) sends the question a player types after `@sai` and nothing else. Neither sends Discord or Minecraft account identifiers. We don't store the chat or the AI's reply beyond delivering it.

If you play on such a server, **this can apply to you whether or not you have ever joined its Discord server**. Ask your server operator whether translation or `@sai` is enabled, and see **Minecraft account linking** above for what is and is not stored.
